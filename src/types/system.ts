// ============================================================
// system.ts — Core type definitions for DES System Builder
// ============================================================
...
export type ProductType =
  ...
  | 'commAccessory'
  | 'commGateway'
  // Inline fuse holders: dc_distribution with fuseSlots but NOT a bus collector
  | 'fuse_holder';
...
export interface TerminalDefinition {
  ...
  /** Communication connector type (RJ45, M12, etc.) — physical property of this jack. */
...
export interface TerminalGroupDefinition {
  ...
  maxFuseA?: number;
  /** Device-internal fuse/breaker protecting this terminal group/conductor. */
  integratedProtection?: IntegratedProtectionDefinition;
...
export interface IntegratedProtectionDefinition {
  /** Protection type: fuse (single-use) or breaker (resettable). */
  protectionType: 'fuse' | 'breaker';
  /** Current rating (A). Required before analysis can count it as verified protection. */
  currentRatingA?: number;
  /** Voltage rating (V). */
  voltageRatingV?: number;
  /** Interrupt/fault current capacity (A). */
  interruptRatingA?: number;
  /** Fuse style when `protectionType` is `fuse`. */
  fuseStyle?: string;
  /** Breaker style when `protectionType` is `breaker`. */
  breakerStyle?: string;
  /** Human-readable protection label. */
  label?: string;
  notes?: string;
}
...
export interface Product {
   ...
   * Optional structured pricing metadata; `msrpUsd` remains the displayed USD value.
   */
