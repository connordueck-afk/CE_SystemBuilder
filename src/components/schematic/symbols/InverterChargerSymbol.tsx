...
export function InverterChargerSymbol({ product, selected }: Props) {
  ...
  return (
    <g>
      ...
      />
      <text x={0} y={-14} textAnchor="middle" fill="#7c61c7" fontSize={10} fontWeight={700}>INV / CHG</text>
      <text x={0} y={0} textAnchor="middle" fill="#46546a" fontSize={8} fontWeight={600}>
        {product.continuousPowerW ? `${(product.continuousPowerW / 1000).toFixed(1)}kW` : ''}
      </text>
      {/* DC label left */}
      <text x={-hw + 4} y={-14} fill="#2f9461" fontSize={7} fontWeight={600}>DC</text>
      {/* AC label right */}
      <text x={hw - 14} y={-14} fill="#d8752b" fontSize={7} fontWeight={600}>AC</text>
      {/* Sine wave symbol */}
